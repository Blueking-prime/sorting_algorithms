 ...
 * @size: size of the array
 *
 * @sIdx: starting index point
 *
 * @eIdx: ending index point
 *
 * Return: void
 */

void quick_sort_recurs(int *array, size_t size, size_t sIdx, size_t eIdx)
{
	size_t pIdx;

	if (eIdx > sIdx)
	{
		pIdx = partition(array, size, sIdx, eIdx);
		if (pIdx != sIdx) /*this if statement is needed*/
			quick_sort_recurs(array, size, sIdx, pIdx - 1);
		quick_sort_recurs(array, size, pIdx + 1, eIdx);
	}
}
